...
async function storeAndRetrievePastMeta(
    ...
    return JSON.parse(
        manager.readString(`${localPath}/${fileToRetrieve}.json`)
    );
}
...
async function hasDeckShiftedPosition(
    ...
    const currentTierPosition = flattenedDecks.findIndex(
        (savedDeck) =>
            (savedDeck as CombinedDeckData).archetype_id === archetypeID
    );
    ...
    return {
        ...
        deckAtSamePosition:
            currentTierPosition === pastTierPosition ? true : false,
        arrowIcon: arrowIcon,
...
async function createTable(
    ...
    for await (const [tier, decks] of Object.entries(deckTiers)) {
        ...
        for await (const deck of decks) {
            ...
            row.onSelect = () => async (number: number) =>
                await viewDeckOnHsReplay(number);
 ...
 */
async function getCurrentAPI(): Promise<0 | 1 | 2> {
    ...
    const currentPatchFilter =
        userData?.features["current-patch-filter"]?.enabled ?? null;

    const currentExpansionFilter =
        userData?.features["current-expansion-filter"]?.enabled ?? null;
...
const allDecksDataRequest: [
    PromiseSettledResult<Archetype[]>,
    PromiseSettledResult<HsReplayDeckData>
...
const archetypes =
    allDecksDataRequest[0].status === "fulfilled"
        ? allDecksDataRequest[0].value
        : null;
const allDecksData =
    allDecksDataRequest[1].status === "fulfilled"
